...
// ============================================================
// CYPHER PROMPT - PHẦN NÂNG CAO (TIẾP THEO)
// Advanced Queries for Study-Visa-Settlement Advisory System
// ============================================================
...
// Use case: "Thống kê chi tiết về từng trường"
MATCH (u:University)
OPTIONAL MATCH (u)-[:HAS_PROGRAMS]->(pg:ProgramGroup)
              -[:HAS_LEVEL]->(pl:ProgramLevel)
              -[:OFFERS]->(p:Program)
...
// Use case: "Ngành nào có nhiều chương trình nhất?"
...
// Use case: "Ngành nào yêu cầu IELTS cao nhất?"
MATCH (cat:StudyCategory)<-[:IN_STUDY_CATEGORY]-(p:Program)
MATCH (p)-[:HAS_REQUIRED]->(es:ExamScore)<-[:HAS_SCORE]-(e:Exam {name: "IELTS"})
WITH cat,
     avg(es.value) AS avg_ielts,
...
// Use case: "Tháng nào có nhiều chương trình nhập học nhất?"
...
// Use case: "Visa nào hỗ trợ nhiều mục đích nhất?"
MATCH (v:Visa)
OPTIONAL MATCH (v)-[:HAS_ABOUT_INFO]->(a:AboutInfo)
...
// Use case: Chatbot nhận câu hỏi: "Tôi muốn học Master IT, IELTS 7.0, có visa gì?"
MATCH (p:Program {program_type: $level})
...
// Use case: "Tìm kiếm mở rộng khi query chính không có kết quả"
MATCH (p:Program)
WHERE toLower(p.name) CONTAINS toLower($keyword)
...
// Kiểm tra xem có phải câu hỏi về visa không
MATCH (v:Visa)
...
// Use case: "Người dùng đã hỏi về UNSW, giờ hỏi 'còn chương trình nào nữa?'"
...
// ============================================================
// CYPHER PROMPT - PHẦN 13, 14, 15
// Optimization, Validation & Export Queries
// ============================================================
...
// Use case: "Chỉ lấy top results sớm nhất có thể"
MATCH (u:University)
WITH u LIMIT 10
...
// Use case: "Tránh query tạo ra quá nhiều combinations"
// ❌ BAD:
// MATCH (u:University), (v:Visa)
// ✅ GOOD:
MATCH (u:University)
...
// Use case: "Chỉ dùng OPTIONAL khi thực sự cần"
MATCH (p:Program)
WHERE p.program_type = $level
...
// Use case: "Tìm program không có yêu cầu đầu vào"
MATCH (p:Program)
...
// Use case: "Tìm Settlement pages không kết nối với category"
MATCH (sp:SettlementPage)
...
// Use case: "Tìm trường bị duplicate"
MATCH (u:University)
...
// Use case: "Kiểm tra Program có đủ relationships cần thiết"
MATCH (p:Program)
...
// Use case: "Tìm các trường dữ liệu null quan trọng"
MATCH (p:Program)
...
// Use case: "Kiểm tra Visa có đầy đủ thông tin"
MATCH (v:Visa)
...
// Use case: "Kiểm tra Visa 500 có kết nối với Study entities"
MATCH (v:Visa {subclass: "500"})
RETURN v.name_visa AS visa,
...
// Use case: "Tìm exam scores bất thường"
MATCH (es:ExamScore)
...
// Use case: "Kiểm tra cấu trúc Settlement đầy đủ"
MATCH (cat:SettlementCategory)
...
// Use case: "Tìm URLs rỗng hoặc không hợp lệ"
MATCH (n)
...
// Use case: "Export toàn bộ chương trình của 1 trường"
MATCH (u:University {name: $university_name})
...
// 15.2. SUMMARY REPORT
// Use case: "Báo cáo tổng quan hệ thống"
MATCH (u:University)
...
// Use case: "Export tất cả thông tin về visa"
MATCH (v:Visa)
OPTIONAL MATCH (v)-[:HAS_ABOUT_INFO]->(a:AboutInfo)
OPTIONAL MATCH (v)-[:HAS_ELIGIBILITY_REQUIREMENT]->(er:EligibilityRequirement)
OPTIONAL MATCH (v)-[:HAS_STEP]->(s:VisaStep)
...
// Use case: "Export cấu trúc Settlement"
MATCH (cat:SettlementCategory)-[:HAS_GROUP]->(tg:SettlementTaskGroup)
      -[:CONTAINS_SETTLEMENT_PAGE]->(sp:SettlementPage)
RETURN cat.name AS Category,
...
// Use case: "Export profile đầy đủ các trường"
MATCH (u:University)
...
// Use case: "Export các mối quan hệ chéo"
MATCH (v:Visa {subclass: "500"})
OPTIONAL MATCH (v)-[:RELEVANT_FOR_UNIVERSITY]->(u:University)
OPTIONAL MATCH (v)-[:ALLOWS_STUDY_LEVEL]->(sl:StudyLevel)
OPTIONAL MATCH (v)-[:HAS_RELEVANT_SETTLEMENT_CATEGORY]->(sc:SettlementCategory)
...
// Use case: "Export thống kê theo ngành"
MATCH (cat:StudyCategory)<-[:IN_STUDY_CATEGORY]-(p:Program)
MATCH (p)<-[:OFFERS]-(pl:ProgramLevel)<-[:HAS_LEVEL]-(pg:ProgramGroup)
      <-[:HAS_PROGRAMS]-(u:University)
...
// Use case: "Export ma trận yêu cầu đầu vào"
...
// Use case: "Export thông tin kỳ nhập học"
MATCH (m:Month)<-[:STARTS_IN]-(p:Program)
MATCH (p)<-[:OFFERS]-(pl:ProgramLevel)<-[:HAS_LEVEL]-(pg:ProgramGroup)
      <-[:HAS_PROGRAMS]-(u:University)
...
// Use case: "Export mẫu cấu trúc graph để visualization"
...
// Use case: "Export dạng JSON cho API"
MATCH (u:University {name: $university_name})
OPTIONAL MATCH (u)-[:HAS_PROGRAMS]->()-[:HAS_LEVEL]->(pl:ProgramLevel)
              -[:OFFERS]->(p:Program)
OPTIONAL MATCH (p)-[:HAS_REQUIRED]->(es:ExamScore)<-[:HAS_SCORE]-(e:Exam)
...
PHẦN 13 - OPTIMIZATION (10 queries):
✅ Indexed lookups
✅ Batch processing
✅ Existence checks
✅ Count optimization
✅ Profile & explain queries
✅ Tránh cartesian products

PHẦN 14 - VALIDATION (10 queries):
✅ Data integrity checks
✅ Orphan nodes detection
✅ Duplicate detection
✅ Relationship validation
✅ NULL value checks
✅ Broken URLs detection

PHẦN 15 - EXPORT & REPORTING (12 queries):
✅ Export programs, visas, settlements
✅ Summary reports
...
// ============================================================
// CYPHER PROMPT - PHẦN 3 (TIẾP THEO)
// More Advanced Queries for Complex Use Cases
// ============================================================
...
// Use case: "Tôi muốn bắt đầu tháng 2/2025, nên chuẩn bị gì từ bây giờ?"
...
// Use case: "Chương trình nào còn nhận đơn trong 2 tháng tới?"
...
// Use case: "Lộ trình từ Bachelor → Master → PR trong 5 năm"
MATCH (bachelor:Program {program_type: "Bachelor"})
...
RETURN {
    ...
    year_5_plus: {
        ...
        visa_options: collect(DISTINCT {
            name: visa_pr.name_visa,
            subclass: visa_pr.subclass
        }),
        graduate_visa: {
            name: visa_graduate.name_visa,
            duration: "2-4 years work rights"
        }
    },
...
// Use case: "Ước tính tổng chi phí học tại Melbourne"
MATCH (u:University {name: $university_name})
      -[:HAS_INFO]->(ip:InfoPage)
      -[:HAS_SECTION]->(cost:InfoSection)
WHERE cost.key CONTAINS 'Cost'
...
WITH u, all_costs, avg(CASE 
    ...
    THEN toFloat(substring(p.description, 
                          apoc.text.indexOf(p.description, '$') + 1, 5))
    ELSE 0 
...
// Use case: "Trường nào rẻ nhất cho Master Business?"
MATCH (p:Program {program_type: $level})
MATCH (p)-[:IN_STUDY_CATEGORY]->(cat:StudyCategory)
WHERE toLower(cat.name) CONTAINS toLower($field)
MATCH (p)<-[:OFFERS]-(pl:ProgramLevel)<-[:HAS_LEVEL]-(pg:ProgramGroup)
...
RETURN u.name AS university,
       ...
       p.url AS url,
       CASE 
           WHEN accommodation_cost IS NOT NULL THEN "Cost info available"
           ELSE "Contact university for costs"
       END AS cost_status
LIMIT 10;

// 17.3. TÌM HỌC BỔNG (nếu có trong description)
// Use case: "Chương trình nào có học bổng?"
MATCH (p:Program)
...
RETURN {
    ...
    local_settlement_info: collect(DISTINCT {
        title: sp.title,
        url: sp.url
    })[..3]
...
// Use case: "Tất cả trường ở từng bang"
MATCH (u:University)
WITH u,
     CASE 
...
// Use case: "Thông tin định cư tại bang Victoria"
MATCH (sp:SettlementPage)
...
// Use case: "Học IT có cơ hội việc làm gì?"
MATCH (cat:StudyCategory)
WHERE toLower(cat.name) CONTAINS toLower($field)
...
// Use case: "Ngành nào thiếu nhân lực ở Úc?"
MATCH (sc:SettlementCategory)
...
// Use case: "Tốt nghiệp Master Business, làm gì tiếp theo?"
MATCH (p:Program {program_type: $level})
...
RETURN {
    ...
    immediate_steps: {
        visa: visa_grad.name_visa,
        purpose: "2-4 years work rights after graduation"
    },
    career_resources: collect(DISTINCT {
        category: sc.name,
        task_group: tg.name
...
// Use case: "So sánh toàn diện 5 trường hàng đầu"
MATCH (u:University)
WHERE u.name IN $university_list
OPTIONAL MATCH (u)-[:HAS_PROGRAMS]->()-[:HAS_LEVEL]->(pl:ProgramLevel)
              -[:OFFERS]->(p:Program)
...
// Use case: "So sánh lộ trình visa 189 vs 190 vs 491"
...
// Use case: "Ngành Engineering ở các trường khác nhau như thế nào?"
MATCH (cat:StudyCategory)
WHERE toLower(cat.name) CONTAINS toLower($field)
...
WITH u, cat,
     ...
     collect(DISTINCT p.program_type) AS levels,
     avg(es.value) AS avg_requirement
RETURN u.name AS university,
       cat.name AS category,
...
// Use case: "melborn university" → "Melbourne"
MATCH (u:University)
...
// Use case: "computer science machine learning AI"
MATCH (p:Program)
...
WITH u, p, subj, cat,
     size([keyword IN $keywords 
          WHERE toLower(p.name) CONTAINS toLower(keyword)]) AS match_count
WHERE match_count > 0
RETURN u.name AS university,
       p.name AS program,
...
// 21.3. SEMANTIC SEARCH (Based on relationships)
// Use case: "Tìm chương trình tương tự với X"
...
MATCH (similar)<-[:OFFERS]-(pl:ProgramLevel)<-[:HAS_LEVEL]-(pg:ProgramGroup)
      <-[:HAS_PROGRAMS]-(u:University)
WITH u, similar,
     CASE WHEN exists((similar)-[:IN_STUDY_CATEGORY]->(cat)) THEN 2 ELSE 1 END 
     AS similarity_score
RETURN u.name AS university,
...
// 22.1. CENTRALITY ANALYSIS (Nodes quan trọng nhất)
// Use case: "StudyCategory nào kết nối nhiều nhất?"
MATCH (cat:StudyCategory)
...
// 22.2. COMMUNITY DETECTION (Nhóm liên quan)
// Use case: "Các trường nào có chương trình tương tự?"
...
// 22.3. PATH ANALYSIS (Phân tích đường đi)
// Use case: "Đường đi ngắn nhất từ Bachelor IT → PR Visa"
MATCH path = shortestPath(
    (start:Program {program_type: "Bachelor"})
    -[*..8]-
    (end:Visa)
)
...
// Use case: "Kiểm tra chất lượng dữ liệu"
MATCH (p:Program)
WHERE p.name IS NULL 
   OR p.name = '' 
   OR NOT exists((p)<-[:OFFERS]-())
RETURN 'Incomplete Program' AS issue,
       count(p) AS count,
       collect(p.uid)[..5] AS examples
UNION
MATCH (v:Visa)
WHERE v.subclass IS NULL OR v.subclass = ''
RETURN 'Incomplete Visa' AS issue,
       count(v) AS count,
       collect(v.name_visa)[..5] AS examples
UNION
MATCH (u:University)
WHERE NOT exists((u)-[:HAS_PROGRAMS]->())
RETURN 'University without programs' AS issue,
       count(u) AS count,
       collect(u.name)[..5] AS examples;

// 23.2. RELATIONSHIP AUDIT
// Use case: "Kiểm tra các mối quan hệ đã tạo"
MATCH ()-[r]->()
...
// Use case: "Thống kê số lượng từng loại node"
...
// 24.1. CREATE INDEXES (Chạy 1 lần duy nhất)
// Use case: "Tối ưu hiệu suất query"
...
// Use case: "Phân tích hiệu suất query"
PROFILE
MATCH (u:University {name: $university_name})
      -[:HAS_PROGRAMS]->()-[:HAS_LEVEL]->()-[:OFFERS]->(p:Program)
RETURN count(p);
...
// 24.3. EXPLAIN PLAN
// Use case: "Xem execution plan"
EXPLAIN
MATCH (v:Visa {subclass: $subclass})
      -[:HAS_ABOUT_INFO]->(a:AboutInfo)
RETURN v, collect(a);
